...
if ($UPDATE_CONFIG_FILE == 0)
{
	...
	if ($old_elements ne "" || $new_elements ne "") {
		$message = "There are differences between products defined in the config file and products defined in Bugzilla:\n"
		...
		$message = "There are inconsistencies found in the products config file:\n"
			. "- classification '$BUGZILLA_CLASSIFICATION'\n"
			. "- products config file: '$PRODUCTS_CONFIG_FILE'\n"
...
else
{
	...
	if ($errors ne "") {
		$message = "Error occured while updating products configuration file:\n"
			. "- classification '$BUGZILLA_CLASSIFICATION'\n"
			. "- products config file: '$PRODUCTS_CONFIG_FILE'\n"
			. "Please review these issues!\n\n$errors";
		fatal ($message);
