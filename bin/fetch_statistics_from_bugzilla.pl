...
#==========================================================================
# BAM (Bugzilla Automated Metrics): fetch_statistics_from_bugzilla.pl
#
...
if ($SUBSET_OF eq "" && $BUGS_DEPENDS_ON_DEPENDENCIES_STR ne "") {
    ...
    $BUGZILLA_URL_BUGS_WITH_DEPENDENCIES_BASE =
      "buglist.cgi?ctype=csv&field0-0-0=dependson&type0-0-0=regexp&value0-0-0=.%2B" . create_bugzilla_param_column_list(@BUGS_DEPENDS_ON_DEPENDENCIES);
    ...
    if ($SUBSET_OF ne "") {
        ...
        if (-e $snapshot_output_dir) {
            if (!-l $snapshot_output_dir) {
                fatal(
"Variable 'SUBSET_OF' is defined, but a product folder '$snapshot_output_dir' is not a symbolic link of parent product folder: '$parent_snapshot_output_dir':");
            }
            # TODO - test this!!
            if (readlink($snapshot_output_dir) ne $parent_snapshot_output_dir) {
                info(
"Variable 'SUBSET_OF' is defined, but a symbolic link is not pointing to the parent product folder '$parent_snapshot_output_dir'. Current situation: '$snapshot_output_dir' -> '"
                      . readlink($snapshot_output_dir)
                      . "' ");
    ...
    print OUTFILE "\$BUGS_ACTIVE = '"
      . (create_bugzilla_params_from_rule(@BUGS_OPEN))[0]
      . (create_bugzilla_params_from_rule(@BUGS_FIXED))[0]
      . (create_bugzilla_params_from_rule(@BUGS_RELEASED))[0] . "';\n";
    print OUTFILE "\$BUGS_NOT_RELEASED = '"
      . (create_bugzilla_params_from_rule(@BUGS_FIXED))[0]
      . (create_bugzilla_params_from_rule(@BUGS_RELEASEABLE))[0] . "';\n";
    print OUTFILE "\$BUGS_OPEN = '" . (create_bugzilla_params_from_rule(@BUGS_OPEN))[0] . "';\n";
    print OUTFILE "\$BUGS_NOT_CONFIRMED = '"
      . (create_bugzilla_params_from_rule(@BUGS_OPEN))[0]
      . (create_bugzilla_params_from_rule(@BUGS_NOT_CONFIRMED))[0] . "';\n";
    print OUTFILE "?>";
    close(OUTFILE);
    ...
    if ($SUBSET_OF ne "") {
        ...
        if (create_bugzilla_param_column_list(@BUGS_CLOSED) eq "&columnlist=,bug_status,") {
            if ($RUN_FEW_TIMES_A_DAY eq "true") {
                ...
                $bugzilla_request_closed .=
                    "&chfield=bug_status&chfieldfrom="
                  . ($day_of_week eq "1" ? "8" : "5")
                  . "d&chfieldto=Now"
                  . (create_bugzilla_params_from_rule(@BUGS_CLOSED))[0];
            }
        }
        else {
        ...
        if (-z "$snapshot_output_dir/$snapshot_output_file") {
            $errors .=
"Statistics file is empty '$snapshot_output_dir/$snapshot_output_file' - statistics has not been fetched correctly from Bugzilla.. - removing the file\n";
                ...
                if ($prev_snapshot_daily eq "none") {
                    ...
                    if ($RUN_FEW_TIMES_A_DAY eq "true") {
                        ...
                        if ($snapshot_date eq $elements[0]) {
                            fatal(
"Statistics has been fetched already today - risk of corupting data! Statistics cannot be run more than once per day, when RUN_FEW_TIMES_A_DAY is not enabled. Please remove all files $STATS_FOLDER/*/raw_data/$snapshot_date*' and fetch statistics once again."
                            );
                        }
                        $prev_snapshot_daily = $s;
                    }
...
$GLOBAL_VARIABLES .= read_global_variable("STR_PRODUCT_COLUMN_NAME");
$GLOBAL_VARIABLES .= read_global_variable("STR_COMPONENT_COLUMN_NAME");
...
sub check_bugs_dependencies() {
    ...
    my $b_id = "";
    my $l    = "";
    my $BUGZILLA_URL_BUGS_WITH_DEPENDENCIES_DETAILS =
"show_bug.cgi?ctype=xml&excludefield=attachment&excludefield=long_desc&excludefield=cc&excludefield=short_desc&excludefield=reporter&excludefield=assigned_to&excludefield=qa_contact&excludefield=token&excludefield=group&excludefield=creation_ts&excludefield=delta_ts&excludefield=keywords&excludefield=status_whiteboard&id=";
    ...
    if (-z "$file") {
        $errors .=
          "Statistics file is empty '$file' - the list of bugs blocking other resolved bugs has not been fetched from Bugzilla..  - removing the file\n";
        $do_continue = 0;
    }
