    ...
    while ($line = <INFILE>) {
        ...
        if ($line_no == 1) {
            ...
            $output_array{$bug_id}{"details"} =
                ";;;$bug_id;;;"
              . $elements[ $columns{"bug_severity"} ] . ";;;"
              . $elements[ $columns{"priority"} ] . ";;;"
              . $elements[ $columns{"short_desc"} ] . "\n";
...
sub save_results {
    ...
    foreach $group_name (keys %results) {
        ...
        print STATSFILE "$DAY;;;$SNAPSHOT_TAKEN_TIME;;;$CHANGES_OUTFILE"
          . ($group_name eq "all_bugs" ? "" : "_$group_name") . ";;;"
          . $results{$group_name}{$STATUS_ACTIVE}{'count'} . ";;;"
          . ($results{$group_name}{$STATUS_ACTIVE}{'count'} - $results{$group_name}{$STATUS_OPEN}{'count'}) . ";;;"
          . $results{$group_name}{$STATUS_VERIFIABLE}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_NOT_RELEASED}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_OPEN}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_NOT_CONFIRMED}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_NEW}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_REOPENED}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_RESOLVED}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_MOVED_OUT}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_RELEASED}{'count'} . ";;;"
          . $results{$group_name}{$STATUS_CLOSED}{'count'} . "\n";
        close STATSFILE;
