...
The new metric must be a subset of existing metric (SUBSET_OF parameter must be defined).
 - read configuration
 - if variable 'SUBSET_OF' is not defined then exit
 - if variable 'SUBSET_OF' is defined then:
      - check if existing symlink pointing to correct folders, if not, then create new ones.
      - next generate historical statistics from 'parent statistics' starting from day <start date>
      - remove last snapshot file
      - and finally run 'fetch_statistics_from_bugzilla.pl' to finish configuration of new statistics
